...
// Función para crear slugs automáticamente
              ...
              <div className="flex transition-transform duration-500 gap-8" style={{ transform: `translateX(-${(index * 100) / 3}%)` }}>
                {proyectos.map((p, i) => (
                  <div key={i} className="flex-none w-1/3 min-w-0 bg-gray-50 p-6 rounded-2xl shadow hover:shadow-lg transition">
                    ...
                    {p.url ? (
                      <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 font-medium hover:underline">
                        Ver proyecto →
                      </a>
                    ) : (
                      <Link href={`/proyectos/${crearSlug(p.title)}`} className="text-blue-600 font-medium hover:underline">
                        Ver más →
                      </Link>
                    )}
                  </div>
...
<section id="contacto" className="py-20 px-6 bg-gray-100">
  ...
  <form
    action="https://formspree.io/f/mzzjdypv" // tu endpoint de Formspree
    method="POST"
  ...
  <a
  href="https://wa.me/529513180462" // +52 para México + 9511234567
  target="_blank"
...
<section id="reseñas" className="py-20 px-6 bg-gray-50">
  ...
  <div className="relative max-w-6xl mx-auto">
    {/* Flechas */}
    <button className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-blue-600 text-white p-3 rounded-full shadow hover:bg-blue-700 transition z-10">&#8592;</button>
    <button className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-blue-600 text-white p-3 rounded-full shadow hover:bg-blue-700 transition z-10">&#8594;</button>
      ...
      <div className="flex transition-transform duration-500 gap-8">
        {resenas.map((r, i) => (
          <div key={i} className="flex-none w-1/3 min-w-0 bg-white p-6 rounded-2xl shadow hover:shadow-lg transition">
            ...
            <span className="text-yellow-500">{"★".repeat(r.estrellas)}</span>
          </div>
        ))}
      </div>
    </div>
